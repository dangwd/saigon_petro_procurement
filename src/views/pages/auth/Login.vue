            ...
            <div
                ...
                <div class="w-full surface-card py-8 px-5 sm:px-8" style="border-radius: 53px">
                    ...
                    <div v-if="typeLog == 'Login'">
                        ...
                        <Button @click="typeLog = 'SignUp'" label="Đăng ký" text class="w-full p-3 text-lg"></Button>
                    </div>
                    <div v-if="typeLog == 'SignUp'">
                        <label for="email1" class="block text-900 text-base font-semibold mb-2">Tên người dùng</label>
                        <InputText id="email1" type="text" placeholder="Nguyen Van A" class="w-full md:w-30rem mb-3"
                            style="padding: 1rem" v-model="userCredentials.username" />
                        <label for="email1" class="block text-900 text-base font-semibold mb-2">Email</label>
                        <InputText id="email1" type="text" placeholder="user.name@gmail.com"
                            class="w-full md:w-30rem mb-3" style="padding: 1rem" v-model="userCredentials.username" />
                        <label for="email1" class="block text-900 text-base font-semibold mb-2">Mật khẩu</label>
                        <Password id="password1" v-model="userCredentials.password" :toggleMask="true"
                            class="w-full mb-3" inputClass="w-full" :inputStyle="{ padding: '1rem' }"></Password>
                        <label for="email1" class="block text-900 text-base font-semibold mb-2">Nhập lại mật
                            khẩu</label>
                        <Password id="password1" v-model="userCredentials.password" :toggleMask="true"
                            class="w-full mb-3" inputClass="w-full" :inputStyle="{ padding: '1rem' }"></Password>
                        ...
                        <Button @click="handleSubmit()" label="Đăng ký" class="w-full p-3 text-lg mb-3"></Button>
                        <Button @click="typeLog = 'Login'" label="Đăng nhập" text class="w-full p-3 text-lg"></Button>
                    </div>
                </div>
            </div>
        </div>
